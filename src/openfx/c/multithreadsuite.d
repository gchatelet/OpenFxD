module openfx.c.multithreadsuite;

import openfx.c.plugincore;

extern(C){

const string kOfxMultiThreadSuite="OfxMultiThreadSuite";

/** @brief Mutex blind data handle
 */
alias void* OfxMutexHandle;
 ...
 */
alias void function( uint threadIndex, uint threadMax, void* customArg ) OfxThreadFunctionV1;
...
struct OfxMultiThreadSuiteV1
{
	 ...
	 *
	 */
	OfxStatus function( OfxThreadFunctionV1 func,
	                            const uint  nThreads,
	                            void* customArg )multiThread;
	 ...
	 */
	OfxStatus function( const uint* nCPUs ) multiThreadNumCPUs;
	 ...
	 */
	OfxStatus function( const uint* threadIndex )multiThreadIndex;
	 ...
	 */
	int function() multiThreadIsSpawnedThread;
	 ...
	 */
	OfxStatus function( OfxMutexHandle* mutex, const int lockCount )mutexCreate; ///@todo; ofxtuttle fix: no const on mutex
	 ...
	 */
	OfxStatus function( OfxMutexHandle mutex )mutexDestroy; ///@todo; ofxtuttle fix: no const on mutex
	 ...
	 * - kOfxStatErrBadHandle - if the handle was bad
	 */
	OfxStatus function( OfxMutexHandle mutex )mutexLock; ///@todo; ofxtuttle fix: no const on mutex
	 ...
	 * - kOfxStatErrBadHandle if the handle was bad
	 */
	OfxStatus function( OfxMutexHandle mutex )mutexUnLock; ///@todo; ofxtuttle fix: no const on mutex
	 ...
	 * - kOfxStatErrBadHandle - if the handle was bad
	 */
	OfxStatus function( OfxMutexHandle mutex )mutexTryLock; ///@todo; ofxtuttle fix: no const on mutex
