module openfx.host.suite.propertysuite;
...
Property getRawProperty(OfxPropertySetHandle properties, const char* property){
	if(properties==null)
		throw new SuiteException(kOfxStatErrBadHandle, "OfxPropertySetHandle must not be null");
	PropertySet* set = cast(PropertySet*)(properties);
	string name = to!string(property);
	if(name !in set.map)
		throw new SuiteException(kOfxStatErrUnknown, "No property with name '"~name~"' in this propertySet");
	return set.map[name];
}
...
T getProperty(T)(OfxPropertySetHandle properties, const char* propertyName){
	...
	if(typed is null)
		throw new SuiteException(kOfxStatErrBadHandle, "The property '"~property.name~"' is of type "~to!string(property.type));
	return typed;
}

void checkBound(T)(in T property, int index){
	if(index >= property.size)
		throw new SuiteException(kOfxStatErrBadIndex, "The property '"~property.name~"' has "~to!string(property.size)~" elements, cannot ask for index "~to!string(index));
}

void checkSize(T)(in T property, int size){
	if(size > property.size)
		throw new SuiteException(kOfxStatErrBadIndex, "The property '"~property.name~"' has only "~to!string(property.size)~" elements");
}
...
// testing null handle
unittest {
	assert ( propReset(null, "") == kOfxStatErrBadHandle );
}

// testing bad name
unittest {
	auto p = new PropertySuite;
	assert ( propReset(&p, null) == kOfxStatErrUnknown );
}
...
unittest {
	...
	s.put(p);
	assert ( propReset(&s, name) == kOfxStatOK );
...
unittest {
	...
	s.put(p);
	int value;
	assert ( propGetInt(&s, name, 0, &value) == kOfxStatOK );
	assert ( value == 1 );
	assert ( propGetInt(&s, name, 1, &value) == kOfxStatErrBadIndex );
}
...
unittest {
	...
	auto p = new IntProperty(name,1);
	s.put(p);
	assert ( propSetInt(&s, name, 0, 12) == kOfxStatOK );
...
unittest {
	...
	auto p = new IntProperty(name,1);
	s.put(p);
	assert ( propSetInt(&s, name, 5, 12) == kOfxStatErrBadIndex );
}
